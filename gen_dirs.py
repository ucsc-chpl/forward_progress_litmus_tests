...
path = '2_threads_2_instructions/'
for num in range(8):
    with open(path + str(num) + '/' + 'index.html', 'w') as file:
        file.write(f"""
...
<body>
  ...
  <script type="module">
    ...
    init().then(() => {{
      ...
      document.getElementById('run_button').addEventListener('click', () => {{
        const resultPromise = wasm_mod.run(2, "2_2_{num}.wgsl");
  ...
  <div class="container">
    <div class="content">
      <img src="{num}.png" alt="state transition diagram" width="500">
    </div>
  </div>
  <div>
    <img src="{num}_code.png" alt="pseudo code" width="300">
  </div>
</body>
</html>
                   """)
