...
def parse_a_exch_br(file, pc, mem_locs, heuristic):
    line = file.readline()
    ...
    if(args['arg_3'] == 'END'):
        ...
        elif heuristic == 'workgroup_barrier' or heuristic == 'workgroup_barrier_random':
        ...
        elif heuristic == 'workgroup_round_robin':
            statement = cust_format(Inst_Strings_Round_Robin.A_EXCH_BR_STR.value, {'pc': pc,'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'arg_2' : args['arg_2'], 'goto' : 'terminate = 1u;'})
        elif heuristic == 'workgroup_chunked':
            statement = cust_format(Inst_Strings_Chunked.A_EXCH_BR_STR.value, {'pc': pc,'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'arg_2' : args['arg_2'], 'goto' : 'terminate = 1u;'})
        elif heuristic == 'workgroup_round_robin_random':
            statement = cust_format(Inst_Strings_Round_Robin.A_EXCH_BR_STR.value, {'pc': pc,'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'arg_2' : args['arg_2'], 'goto' : 'terminate = 1u;'})
        elif heuristic == 'workgroup_chunked_random':
            statement = cust_format(Inst_Strings_Chunked.A_EXCH_BR_STR.value, {'pc': pc,'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'arg_2' : args['arg_2'], 'goto' : 'terminate = 1u;'})
        else:
...
def parse_a_chk_br(file, pc, mem_locs, heuristic='single'):
    ...
    if (args['arg_2'] == 'END'):
        ...
        if heuristic == 'single':
            statement = cust_format(Inst_Strings_Single.A_CHK_BR_STR.value, {'pc': pc, 'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'goto' : f'pc = {(args["arg_2"])}u;'})
        elif heuristic == 'round_robin':
            statement = cust_format(Inst_Strings_Round_Robin.A_CHK_BR_STR.value, {'pc': pc, 'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'goto' : f'pc = {(args["arg_2"])}u;'})
        elif heuristic == 'chunked':
            statement = cust_format(Inst_Strings_Chunked.A_CHK_BR_STR.value, {'pc': pc, 'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'goto' : f'pc = {(args["arg_2"])}u;'})
        elif heuristic == 'workgroup_barrier'or heuristic == 'workgroup_barrier_random':
            statement = cust_format(Inst_Strings_Workgroup_Barrier.A_CHK_BR_STR.value, {'pc': pc, 'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'goto' : f'pc = {(args["arg_2"])}u;'})
        elif heuristic == 'workgroup_round_robin':
            statement = cust_format(Inst_Strings_Round_Robin.A_CHK_BR_STR.value, {'pc': pc, 'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'goto' : f'pc = {(args["arg_2"])}u;'})
        elif heuristic == 'workgroup_chunked':
            statement = cust_format(Inst_Strings_Chunked.A_CHK_BR_STR.value, {'pc': pc, 'arg_0': args['arg_0'], 'arg_1': args['arg_1'], 'goto' : f'pc = {(args["arg_2"])}u;'})
...
def parse_a_st(file, pc, mem_locs, heuristic='single'):
    ...
    if heuristic == 'single':
        ...
        print('uh oh! unsupported heuristic!')
        exit(1)
    return statement
...
def parse_thread(wgsl_kernel, thread_id, file, mem_locs, target_file, heuristic):
    ...
    if heuristic == 'single':
        ...
        thread = cust_format(BP_Strings.WORKGROUP_ROUND_ROBIN_THREAD_STR.value, {'thread_id':thread_id, 'pc_insts':pc_insts, 'last_pc':pc})
    elif heuristic == 'workgroup_chunked':
        thread = cust_format(BP_Strings.WORKGROUP_CHUNKED_THREAD_STR.value, {'thread_id':thread_id, 'pc_insts':pc_insts, 'last_pc':pc})
    elif heuristic == 'workgroup_round_robin_random':
        thread = cust_format(BP_Strings.WORKGROUP_ROUND_ROBIN_RANDOM_THREAD_STR.value, {'thread_id':thread_id, 'pc_insts':pc_insts, 'last_pc':pc})
    elif heuristic == 'workgroup_chunked_random':
        thread = cust_format(BP_Strings.WORKGROUP_CHUNKED_RANDOM_THREAD_STR.value, {'thread_id':thread_id, 'pc_insts':pc_insts, 'last_pc':pc})
    return thread
...
def gen_wgsl_workgroup_round_robin(target_file, wgsl_name):
    with open('wgsl_template.wgsl', 'r') as wgsl_template_file:
        wgsl_template_str = wgsl_template_file.read()
        print(f"parsing target file : {target_file}")
        mem_locs = set()
        ...
        with open(target_file, 'r') as file:
            while (True):
                ...
                if (line == ''):
                    ...
                    args = re.match(Inst_REs.TD_RE.value, line)
                    thread_code_str += parse_thread(thread_code_str, args['tid'], file, mem_locs, target_file, 'workgroup_round_robin')
                    num_threads += 1
            file.close()
        mem_locs_str = ''
        for loc in mem_locs:
        ...
        replacements = {'mem_locs' : mem_locs_str,
                        'workgroup_size' : r'@workgroup_size(256, 1, 1)',
                        'setup' : BP_Strings.BOILER_PLATE_WORKGROUP_BARRIER_STR.value + cust_format(BP_Strings.BOILER_PLATE_WORKGROUP_ROUND_ROBIN_STR.value,{'num_testing_threads': num_threads}),
                        'thread_code' : thread_code_str,
                        'syncronize' : 'workgroupBarrier();'}
...
def gen_wgsl_workgroup_chunked(target_file, wgsl_name):
    with open('wgsl_template.wgsl', 'r') as wgsl_template_file:
        wgsl_template_str = wgsl_template_file.read()
        print(f"parsing target file : {target_file}")
        mem_locs = set()
        ...
        with open(target_file, 'r') as file:
            while (True):
                ...
                if (line == ''):
                    ...
                    args = re.match(Inst_REs.TD_RE.value, line)
                    thread_code_str += parse_thread(thread_code_str, args['tid'], file, mem_locs, target_file, 'workgroup_chunked')
                    num_threads += 1
            file.close()
        mem_locs_str = ''
        for loc in mem_locs:
        ...
        replacements = {'mem_locs' : mem_locs_str,
                        'workgroup_size' : r'@workgroup_size(256, 1, 1)',
                        'setup' : BP_Strings.BOILER_PLATE_WORKGROUP_BARRIER_STR.value + cust_format(BP_Strings.BOILER_PLATE_WORKGROUP_CHUNKED_STR.value,{'num_testing_threads': num_threads}),
                        'thread_code' : thread_code_str,
                        'syncronize' : 'workgroupBarrier();'}
...
def gen_wgsl_workgroup_round_robin_random(target_file, wgsl_name):
    with open('wgsl_template.wgsl', 'r') as wgsl_template_file:
        wgsl_template_str = wgsl_template_file.read()
        print(f"parsing target file : {target_file}")
        mem_locs = set()
        ...
        with open(target_file, 'r') as file:
            while (True):
                ...
                if (line == ''):
                    ...
                    args = re.match(Inst_REs.TD_RE.value, line)
                    thread_code_str += parse_thread(thread_code_str, args['tid'], file, mem_locs, target_file, 'workgroup_round_robin_random')
                    num_threads += 1
            file.close()
        mem_locs_str = ''
        for loc in mem_locs:
...
def gen_wgsl_workgroup_chunked_random(target_file, wgsl_name):
    with open('wgsl_template.wgsl', 'r') as wgsl_template_file:
        wgsl_template_str = wgsl_template_file.read()
        print(f"parsing target file : {target_file}")
        mem_locs = set()
        ...
        with open(target_file, 'r') as file:
            while (True):
                ...
                if (line == ''):
                    ...
                    args = re.match(Inst_REs.TD_RE.value, line)
                    thread_code_str += parse_thread(thread_code_str, args['tid'], file, mem_locs, target_file, 'workgroup_chunked_random')
                    num_threads += 1
            file.close()
        mem_locs_str = ''
        for loc in mem_locs:
...
def gen_wgsl(target_file, wgsl_name='test', heuristic='single'):
    if heuristic == 'single':
        ...
        gen_wgsl_workgroup_barrier_random(target_file, wgsl_name)
    elif heuristic == 'workgroup_round_robin':
        gen_wgsl_workgroup_round_robin(target_file, wgsl_name)
    elif heuristic == 'workgroup_chunked':
        gen_wgsl_workgroup_chunked(target_file, wgsl_name)
    elif heuristic == 'workgroup_round_robin_random':
        gen_wgsl_workgroup_round_robin_random(target_file, wgsl_name)
    elif heuristic == 'workgroup_chunked_random':
        gen_wgsl_workgroup_chunked_random(target_file, wgsl_name)
    else:
...
def main():
    ...
    parser.add_argument('-t', '--test', help='for testing, ignore', default = None)
